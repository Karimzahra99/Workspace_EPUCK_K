...
int main(void)
{
	...
	//timer7_start(); //comment to use selector!!! sinon les interrupt vont tout casser
	...
	//	while(1){//main for timer interrupt //comment to use selector //used to blink led7 with timer7
	//		;
	//	}
...
//	}
	...
	uint32_t delay_dev = 32; // code to change speed of pattern with selector
	while (1) {
		delay(SystemCoreClock/delay_dev);
		selector = get_selector();
		if (selector != old_selector) {
			sequence_pos = 0;
			gpio_set(LED1);
